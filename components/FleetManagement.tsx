  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="flex bg-white p-1.5 rounded-[2rem] shadow-sm border border-slate-200 mb-8 w-fit overflow-x-auto max-w-full">
        {[{ id: 'overview', label: 'Dashboard', icon: LayoutGrid }, { id: 'vehicles', label: 'Véhicules', icon: Truck }, { id: 'drivers', label: 'Gestion d\'Équipe', icon: UserCheck }, { id: 'fuel', label: 'Carburant', icon: Fuel }, { id: 'accidents', label: 'Sinistres', icon: AlertTriangle }, { id: 'reports', label: 'Analytique', icon: BarChart3 }].map(tab => (
      ...
      {isLoading ? <div className="py-32 flex flex-col items-center justify-center"><Loader2 className="w-10 h-10 text-orange-500 animate-spin mb-4" /></div> : (
        <div className="animate-in fade-in duration-500">
          ...
          {activeTab === 'drivers' && (
            <div className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm">
              <div className="p-8 border-b flex justify-between items-center"><h3 className="text-xl font-black uppercase italic tracking-tighter">Gestion d'Équipe</h3><button onClick={() => { setEditingId(null); setDriverForm({ status: 'active', attachments: [], name: '', licenseNumber: '', phone: '', joinDate: new Date().toISOString().split('T')[0] }); setShowModal('drivers'); }} className="px-6 py-3 bg-slate-900 text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg">Nouveau</button></div>
              <div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-slate-50 text-[10px] font-black uppercase text-slate-400"><tr><th className="px-8 py-5">Nom</th><th className="px-8 py-5">Docs</th><th className="px-8 py-5 text-right">Statut</th><th className="px-8 py-5 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{drivers.map(d => <tr key={d.id} className="hover:bg-slate-50 transition-all"><td className="px-8 py-6"><p className="font-black text-slate-900">{d.name}</p><p className="text-[10px] text-slate-400 uppercase">{d.phone}</p></td><td className="px-8 py-6"><div className="flex -space-x-2">{d.attachments?.slice(0, 3).map((a, i) => <button key={i} onClick={() => openAttachment(a)} className="w-8 h-8 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-[8px] font-black" title={a.name}>{a.mimeType.includes('pdf') ? 'PDF' : 'IMG'}</button>)}</div></td><td className="px-8 py-6 text-right"><span className={`px-2 py-1 rounded text-[8px] font-black uppercase ${d.status === 'active' ? 'bg-emerald-50 text-emerald-600' : d.status === 'on_leave' ? 'bg-amber-50 text-amber-600' : 'bg-rose-50 text-rose-600'}`}>{d.status}</span></td><td className="px-8 py-6 text-right"><button onClick={() => handleEdit('drivers', d)} className="p-2.5 text-slate-300 hover:text-blue-500"><Edit2 className="w-4 h-4" /></button></td></tr>)}</tbody></table></div>
            </div>
          )}
